...
MatrixXd SCManager::makeScancontext(const std::vector<Eigen::Vector3d> &_scan_down)
{
    ...
    for (const auto &point3d : _scan_down)
    {
        // xyz to ring, sector
        azim_range = sqrt(point3d.x() * point3d.x() + point3d.y() * point3d.y());
        azim_angle = xy2theta(point3d.x(), point3d.y());
        ...
        // taking maximum z
        if (desc(ring_idx - 1, sctor_idx - 1) < point3d.z() + LIDAR_HEIGHT) // -1 means cpp starts from 0
            desc(ring_idx - 1, sctor_idx - 1) = point3d.z() + LIDAR_HEIGHT; // update for taking maximum value at that bin
    }
...
std::pair<int, float> SCManager::detectLoopClosureID(void)
{
    ...
    if (tree_making_period_conter % TREE_MAKING_PERIOD_ == 0) // to save computation cost
    {
        ...
        // tree_point3dr_->index->buildIndex(); // inernally called in the constructor of InvKeyTree (for detail, refer the nanoflann and KDtreeVectorOfVectorsAdapoint3dor)
    }
